...
def main():
    ...
    trainer=Trainer(model=model,args=TrainingArguments(output_dir=str(args.output),num_train_epochs=args.epochs,max_steps=args.max_steps,per_device_train_batch_size=1,gradient_accumulation_steps=8,logging_steps=10,save_strategy="epoch",eval_strategy="epoch" if tokenized_validation is not None and not args.no_eval else "no",report_to=[]),train_dataset=tokenized,eval_dataset=None if args.no_eval else tokenized_validation,data_collator=collate)
    trainer.train(); model.save_pretrained(args.output); tokenizer.save_pretrained(args.output); print(f"Saved adapter to {args.output}"); return 0
